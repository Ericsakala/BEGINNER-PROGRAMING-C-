...
#include <iostream>  //helps us to acces input and output to the screen
...
void addinnovator() {//function for collecting data from the user and write them to the file
    ...
    ofstream file;
    file.open("record.txt", ios::app);//ios::app appends and avoid overwriting in a file
...
void display() { //the function for reading and displaying all data available in the file calles new record
  ...
  while(!read.eof()) { //if is still reading and havent reached at the end of file
      ...
      //	cout<<"no record found\n";
        ...
        //calling the function for displaying data that has been read
        // cout<<endl;   
     }
...
void updaterecord() {
    ...
    if (choice == 1) {
        Innovator newinnovator;
        cout << "Enter your first name: ";
    getline(cin, newinnovator.firstName);//geting data from the user
    cout << "Enter your surname: ";
    ...
    cin.ignore();
    cout << "Enter your NRC/PASSPORT number: ";
    getline(cin, newinnovator.nrc);
      cout << "Patent Status (patented,in progress, or not required): ";
    getline(cin,newinnovator.status);
